...
public class FileManager extends AbstractVerticle
{
    ...
    public FileManager(Constants.ApplicationType applicationType, String ip, int pingPort)
    {
        this.applicationType = applicationType;

        ApplicationContextStore.setAppContext(applicationType, ip, pingPort);
        ...
        try
        {
            vertx.deployVerticle(new HeartbeatReceiver(applicationType, applicationContext.getString("ip"), applicationContext.getInteger("ping.port")))
                    .compose(result ->
                    {
                        ...
                        {
                            vertx.eventBus()
                                    .<Long>localConsumer(Constants.EVENT_HEARTBEAT + applicationType, message -> timeStamp = message.body());
                            ...
                            vertx.eventBus()
                                    .<String>localConsumer(Constants.EVENT_NEW_FILE, file -> fileQueue.add(file.body()));
    ...
    private void processNextFile(AtomicInteger events)
    {
        try
        {
            if (!fileQueue.isEmpty() && events.get() < MAX_EVENTS)
            {
                ...
                Util.isFileCreatedAtLeastNMinutesAgo(currentFile, 2)
                        .compose(response ->
                        {
                            if (currentFile != null)
                            {
                                ...
                                vertx.eventBus()
                                        .request(Constants.EVENT_READ_FILE, new JsonObject().put("file.name", currentFile)
                                                .put("offset", applicationContext.getLong("offset", 0L)))
                                        .onComplete(request ->
                                        {
                                            try
                                            {
                                                    ...
                                                    else
                                                    {
                                                        if (isAlive())
                                                        {
                                                            ...
                                                            if (FileStatusTracker.readByAllApps(currentFile))
                                                            {
                                                                ...
                                                                vertx.eventBus()
                                                                        .send(Constants.EVENT_CLOSE_FILE, currentFile);
                                                            }

                                                            fileQueue.poll();

                                                            if (!fileQueue.isEmpty())
                                                                applicationContext.put("current.file", fileQueue.peek())
                                                                        .put("offset", 0);
    ...
    public boolean send(String line)
    {
        try
        {
            if (isAlive())
            {
                ...
                vertx.eventBus().send(Constants.EVENT_SEND,applicationType.toString().toLowerCase() + " " + line);
